        ...
        Pcf_method((
          ...
          Cfk_concrete(
            _,
            {
              pexp_desc:
                Pexp_poly(
                  {pexp_desc: Pexp_fun(_, _, _, {pexp_desc: Pexp_constraint({pexp_desc: a}, _)})},
                  None
                )
            }
          )
...
let rec statementBlockMapper =
        (~context, {Parser_flow.Ast.Statement.Block.body}: Parser_flow.Ast.Statement.Block.t)
...
and functionMapper =
    (
      ~context,
      ~returnType,
      {Parser_flow.Ast.Function.id, params: (params, restParam), body, expression, _}
    ) => {
      ...
      |> List.fold_left(
           (expr', (_, param)) =>
             ...
             | Pattern.Expression(_) =>
               Exp.fun_("", None, Pat.var(astHelperStrLidStr("fixme")), expr')
             },
           bodyReason
         );
    switch first {
...
and jsxChildMapper = (~context, (_, child)) =>
  Parser_flow.Ast.JSX.(
    ...
    | Parser_flow.Ast.JSX.ExpressionContainer({ExpressionContainer.expression}) =>
      switch expression {
        ...
        | Pattern.Object({Pattern.Object.properties}) =>
          ...
          | _ =>
            Exp.let_(
              ...
              [
                parseTreeValueBinding(
                  ~pat=Pat.constant(Const_string("destructuringNotImplemented", None)),
                  ~expr
                )
              ],
              innerMostExpr
